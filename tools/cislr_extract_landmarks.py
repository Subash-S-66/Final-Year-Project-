...
# Allow running as: python tools/cislr_extract_landmarks.py
...
def _process_one(
    ...
    np.savez_compressed(
        ...
        person_id=np.array("cislr"),
        dataset=np.array("cislr"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extract MediaPipe landmarks from downloaded CISLR videos into standardized .npz samples.")
    parser.add_argument("--videos", default="data/raw/cislr/videos", help="Input videos root: <LABEL>/*.mp4")
    parser.add_argument("--out", default="data/processed/cislr", help="Output samples root")
    parser.add_argument("--vocab", default="configs/vocab_v1_51.json")
    parser.add_argument("--T", type=int, default=30)
    parser.add_argument("--fps", type=float, default=15.0)
    parser.add_argument("--workers", type=int, default=1, help="Processes for extraction (1 is safest)")
    parser.add_argument("--no-cache", action="store_true", help="Disable caching")
    parser.add_argument("--delete-videos-after-extract", action="store_true", help="Delete raw videos after successful extraction")
    parser.add_argument(
        "--confirm-alignment",
        action="store_true",
        help="Required for full extraction. Without this flag we will do a small dry-run.",
    )
    parser.add_argument(
        "--dry-run-limit",
        type=int,
        default=2,
        help="If not confirmed, process at most N videos per token.",
    )
    ...
    # Build job list from folder structure: data/raw/cislr/videos/<LABEL>/<UID>.mp4
    jobs: list[tuple[str, str, int, str, str]] = []  # (video_path, token, y, uid, split)
    for label_dir in videos_root.iterdir():
        if not label_dir.is_dir():
            continue
        token_name = label_dir.name
        if token_name not in vocab_to_id:
            # Ignore classes not in vocab
            continue

        clips = sorted(label_dir.glob("*.mp4")) + sorted(label_dir.glob("*.webm")) + sorted(label_dir.glob("*.mov"))
        if not args.confirm_alignment:
            clips = clips[: args.dry_run_limit]

        for vp in clips:
            y = int(vocab_to_id[token_name])
            uid = vp.stem
            # Split isn't guaranteed in this storage layout; keep placeholder.
            split = "unknown"
            jobs.append((str(vp), token_name, y, uid, split))
    ...
    if not args.confirm_alignment:
        print(
            "\nAlignment not confirmed: running DRY-RUN extraction. "
            "Review outputs and then re-run with --confirm-alignment for full extraction.\n"
        )
    ...
    if args.workers <= 1:
        for video_path, token_name, y, uid, split in tqdm(jobs, desc="extract", unit="clip"):
            try:
                _process_one(
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            futs = []
            for video_path, token_name, y, uid, split in jobs:
                futs.append(
                    ex.submit(
                        _process_one,
                        video_path,
